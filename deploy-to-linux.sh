...
# Cloudedze Linux Deployment Script
...
SERVER_PATH="/opt/infraedze"  # Change to your desired deployment path
APP_PORT="3000"  # Change if you want a different port
DOMAIN="your-domain.com"  # Change to your domain (optional)

echo -e "${BLUE}🚀 Cloudedze Linux Deployment Script${NC}"
echo "================================================"
...
if [ "$1" = "" ] || [ "$2" = "" ]; then
    ...
    print_info "Example: $0 192.168.1.100 ubuntu /opt/infraedze"
    exit 1
fi

SERVER_HOST=$1
SERVER_USER=$2
SERVER_PATH=${3:-"/opt/infraedze"}
...
sudo apt install -y curl wget git nginx postgresql postgresql-contrib python3 python3-pip python3-venv nodejs npm build-essential
...
sudo tee /etc/nginx/sites-available/cloudedze << EOF
server {
    listen 80;
...
# Enable site
sudo ln -sf /etc/nginx/sites-available/cloudedze /etc/nginx/sites-enabled/
